...
*SeekMem 与 [OpenClaw](https://github.com/openclaw-ai/openclaw) 集成：为 AI 智能体提供智能记忆。**OpenClaw SeekMem 记忆插件**：[查看插件](https://github.com/ob-labs/openclaw-extension-seekmem)*

<img src="docs/images/openclaw_seekmem.jpeg" alt="SeekMem 与 OpenClaw" width="900"/>
...
<img src="docs/images/benchmark_metrics_cn.svg" alt="SeekMem LOCOMO 压测指标" width="900"/>
...
在 AI 应用开发中，如何让大语言模型持久化地"记住"历史对话、用户偏好和上下文信息是一个核心挑战。SeekMem 融合向量检索、全文检索和图数据库的混合存储架构，并引入认知科学的艾宾浩斯遗忘曲线理论，为 AI 应用构建了强大的记忆基础设施。系统还提供完善的多智能体支持能力，包括智能体记忆隔离、跨智能体协作共享、细粒度权限控制和隐私保护机制，让多个 AI 智能体能够在保持独立记忆空间的同时实现高效协作。

## 🚀 核心特性

### 👨‍💻 开发者友好
- 🔌 **[轻量级接入方式](docs/examples/scenario_1_basic_usage.md)**：提供简洁的 Python SDK 支持，自动从 `.env` 文件加载配置，让开发者快速集成到现有项目中。还支持 [CLI](docs/guides/0012-cli_usage.md)（`smem`）、[MCP Server](docs/api/0004-mcp.md) 和 [HTTP API Server](docs/api/0005-api_server.md) 三种接入方式
...
SeekMem 提供命令行工具 `smem`，无需编写 Python 代码即可进行记忆操作、配置管理、备份/恢复以及交互式 shell。

```bash
# 添加与搜索记忆
smem memory add "用户偏好深色模式" --user-id user123
smem memory search "偏好" --user-id user123

# 配置与统计
smem config show
smem config init          # 交互式 .env 配置向导
smem stats --json
...
SeekMem 提供生产就绪的 HTTP API Server，通过 RESTful API 暴露所有核心记忆管理功能。同时提供 **Dashboard**（路径 `/dashboard/`）的 Web 管理界面。

**与 SDK 的关系**：API Server 底层使用相同的 SeekMem SDK，并共享相同的配置（`.env` 文件）。它提供了与 Python SDK 相同的记忆管理功能的 HTTP 接口，使 SeekMem 可供非 Python 应用程序使用。
...
SeekMem 还提供了模型上下文协议（MCP Server），支持与 Claude Desktop 等 MCP 兼容客户端集成。MCP Server通过 MCP 协议暴露 SeekMem 的记忆管理功能，使 AI 助手能够无缝访问和管理记忆。

**与 SDK 的关系**：MCP Server 使用相同的 SeekMem SDK 并共享相同的配置（`.env` 文件）。它提供了与 Python SDK 相同的记忆管理功能的 MCP 接口，使 SeekMem 可供 MCP 兼容的 AI 助手使用。
...
## 🔗 集成与演示
- 🔗 **openclaw 外挂记忆插件**：在 [openclaw](https://github.com/openclaw/openclaw) 中通过插件方式使用 SeekMem 长期记忆，支持智能抽取、艾宾浩斯遗忘曲线、多 Agent 隔离。[查看插件](https://github.com/ob-labs/openclaw-extension-seekmem)
- 🔗 **LangChain 集成**：基于 LangChain + SeekMem + OceanBase 构建医疗支持机器人，[查看示例](examples/langchain/README.md)
- 🔗 **LangGraph 集成**：基于 LangGraph + SeekMem + OceanBase 构建客户服务机器人，[查看示例](examples/langgraph/README.md)

## 📚 文档

- 📖 **[入门指南](docs/guides/0001-getting_started.md)**：安装和快速开始指南
- ⌨️ **[CLI 使用指南](docs/guides/0012-cli_usage.md)**：SeekMem CLI（smem）参考（1.0.0+）
...
|---------|-------|---------|
| 1.0.0 | 2026.03.16 | <ul><li>SeekMem CLI (smem)：记忆操作、配置管理、备份/恢复/迁移、交互式 shell、shell 补全</li><li>Web Dashboard：记忆管理与可视化</li></ul> |
...
- 🐛 **问题反馈**：[GitHub Issues](https://github.com/oceanbase/seekmem/issues)
- 💭 **讨论交流**：[GitHub Discussions](https://github.com/oceanbase/seekmem/discussions)
