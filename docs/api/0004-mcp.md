...
SeekMem MCP Server - Model Context Protocol server for SeekMem memory management.


## Prerequisites

Before using the SeekMem MCP Server, please ensure:

1. **SeekMem is installed**: The server requires SeekMem to be installed. You can install it via:
   ```shell
   pip install seekmem
   ```

2. **Configuration file exists**: Create a `.env` file in your working directory or project root with SeekMem configuration. The server will automatically search for `.env` files in:
   ...
   ```shell
   cp seekmem_mcp/.env.example .env
   ```
...
For SeekMem installation and configuration, please refer to:

[SeekMem Documentation](https://www.seekmem.ai/docs)

## Startup

### Support for multiple types of MCP

You can start SeekMem MCP with different protocols using the following commands:

```shell
uvx seekmem-mcp sse # sse mode, default port 8000 (recommended)
uvx seekmem-mcp stdio # stdio mode
uvx seekmem-mcp sse 8001 # sse mode, specify port 8001
uvx seekmem-mcp streamable-http # streamable-http mode, default port 8000
uvx seekmem-mcp streamable-http 8001 # streamable-http mode, specify port 8001
```
...
The SeekMem MCP Server provides the following memory management tools:
...
When you need help, you can find developers and other community partners at [https://github.com/oceanbase/seekmem](https://github.com/oceanbase/seekmem).

When you discover project defects, please create a new issue on the [issues](https://github.com/oceanbase/seekmem/issues) page.
