"""
SeekMem CLI Configuration Commands
...
def _load_config_with_env_file(env_file: Optional[str]) -> Dict[str, Any]:
    ...
    if env_file:
        os.environ["SEEKMEM_ENV_FILE"] = env_file
    return auto_config()
...
def validate_cmd(ctx: CLIContext, env_file, strict, json_output, check_connectivity):
    ...
    Examples:
        ...
        smem config validate --env-file .env.production
        smem config validate --strict
        smem config validate --check-connectivity
    """
    ctx.json_output = ctx.json_output or json_output
    try:
...
def test_cmd(ctx: CLIContext, component, json_output):
    ...
    Examples:
        smem config test
        smem config test --component database
        smem config test --component llm
    """
    ctx.json_output = ctx.json_output or json_output
    results = {
        "database": None,
...
def _resolve_default_env_file() -> str:
    try:
        from seekmem.settings import _DEFAULT_ENV_FILE  # type: ignore
...
def _discover_env_example(start_dir: Path, max_parent_levels: int = 8) -> Optional[Path]:
    ...
    This keeps `smem config init` convenient when launched from a subdirectory.
    """
...
def _wizard_database(existing: Dict[str, str]) -> Dict[str, str]:
    ...
    if provider == "sqlite":
        updates["SQLITE_PATH"] = click.prompt(
            "SQLite path",
            default=existing.get("SQLITE_PATH") or "./data/seekmem_dev.db",
            show_default=True,
        )
    ...
    if provider == "oceanbase":
        ...
        updates["OCEANBASE_DATABASE"] = click.prompt(
            ...
            default=existing.get("OCEANBASE_DATABASE") or "seekmem",
            show_default=True,
        )
...
def init_cmd(ctx: CLIContext, env_file: Optional[str], dry_run: bool, test: bool, component: str, advanced: bool):
    ...
    Examples:
        smem config init
        smem config init -f .env.production
        smem config init --dry-run
    ...
    click.echo("=" * 60)
    click.echo("SeekMem Interactive Configuration")
    click.echo("=" * 60)
    ...
    if test:
        ...
        try:
            os.environ["SEEKMEM_ENV_FILE"] = target_path
            ...
            # (kept consistent with `smem config test`)
            if component in ("database", "all"):
                print_info("Testing database connection...")
                try:
