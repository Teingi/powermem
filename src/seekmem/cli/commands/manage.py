"""
SeekMem CLI Management Commands
...
@click.option(
    ...
    type=click.Path(),
    default=None,
    help="Output file path (default: seekmem_backup_<timestamp>.json)"
)
...
def backup_cmd(ctx: CLIContext, output, user_id, agent_id, run_id, limit, include_metadata, json_output):
    ...
    \b
    Examples:
        smem manage backup --output backup.json
        smem manage backup --user-id user123 --output user_backup.json
        smem manage backup --limit 1000
    """
    ...
    try:
        ...
        if not output:
            ...
            output = f"seekmem_backup_{timestamp}.json"
...
def restore_cmd(ctx: CLIContext, input_file, user_id, agent_id, dry_run, skip_duplicates, json_output):
    ...
    \b
    Examples:
        smem manage restore --input backup.json
        smem manage restore --input backup.json --dry-run
        smem manage restore --input backup.json --user-id new_user
    """
    ctx.json_output = ctx.json_output or json_output
    try:
...
def cleanup_cmd(ctx: CLIContext, threshold, archive_threshold, user_id, agent_id, dry_run, force, json_output):
    ...
    Examples:
        ...
        smem manage cleanup --user-id user123 --force
    """
    ctx.json_output = ctx.json_output or json_output
    try:
...
def migrate_cmd(ctx: CLIContext, target_store, source_store, delete_source, dry_run, json_output):
    ...
    \b
    Examples:
        smem manage migrate --target-store 0 --dry-run
        smem manage migrate --target-store 1 --delete-source
    """
    ctx.json_output = ctx.json_output or json_output
    try:
