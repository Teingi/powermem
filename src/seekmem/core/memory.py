...
def _auto_convert_config(config: Dict[str, Any]) -> Dict[str, Any]:
    """
    Convert legacy seekmem config to format for compatibility.
    
    Now seekmem uses field names directly.
    ...
    # Check if legacy seekmem format (has database or embedding)
    if "database" in config or ("llm" in config and "embedding" in config):
        ...
        logger.info("Converted legacy seekmem config format")
        return converted
...
class Memory(MemoryBase):
    ...
    def __init__(
        ...
        Args:
            config: Configuration dictionary or MemoryConfig object containing all settings.
                   ...
                   and seekmem style (database, llm, embedding)
        ...
        Example:
            ...
            # Method 2: Using dict (backward compatible - seekmem style)
            memory = Memory({
    ...
    def get_statistics(
        ...
        CLI (smem stats) and Dashboard show identical results.
